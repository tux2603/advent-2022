...
class Monkey:
    ...
    def step(self):
        while self.queue:
            ...
            item = self.worry_lambda(item) % Monkey.worry_modulo
            target = self.target_lambda(item)
            self.inspections += 1
            yield (target, item)

monkeys = []
Monkey.worry_modulo = 1
